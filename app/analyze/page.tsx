...
export default function AnalyzePage() {
  ...
  useEffect(() => {
    if (status === 'unauthenticated') {
      router.replace('/auth/signin?from=/analyze');
    }
  ...
  const handleImageSelected = async (file: File) => {
    // Check if user is premium or has free analyses remaining
    if (!session?.user?.isPremium) {
      const currentMonth = new Date().getMonth();
      const freeAnalysesThisMonth = history.filter(a => {
        const analysisMonth = new Date(a.timestamp).getMonth();
        return analysisMonth === currentMonth;
      }).length;

      if (freeAnalysesThisMonth >= 5) {
        setError('You\'ve reached your free analysis limit for this month. Upgrade to Premium for unlimited analyses.');
        return;
      }
    }
  ...
  const remainingFreeAnalyses = Math.max(0, 5 - freeAnalysesThisMonth);
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900">
      ...
      <main className="max-w-4xl mx-auto px-4 py-6 pb-20">
        ...
        {!session?.user?.isPremium && (
          <div className="mb-6 bg-gradient-to-r from-blue-900/50 to-purple-900/50 border border-blue-700/50 rounded-lg p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-white font-semibold">
                  Free Plan: {remainingFreeAnalyses} / 5 analyses remaining this month
                </p>
